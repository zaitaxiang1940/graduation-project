...
import logging
import os
from trajectory.utils.paths import LOGS_DIR, RUNS_DIR, resolve_runtime_path
...
class Settings:
    ...
    MERGE_POINT_X = -50
    CAR_LENGTH = 5.0
    USE_ALTERNATE_TRAFFIC_DISTRIBUTION = True
    USE_SIMPLE_TRAFFIC_DISTRIBUTION = False
    ...
    # Reward Functions
    REWARD_FUNCTION = "Slotted Jerk"           # "Continuous" or "Slotted" or "ST" or "Slotted Jerk" or "Shaped" or "Shaped Stable"
    ...
    # Reward shaping (dense + potential-based)
    SHAPING_GAMMA = 0.999
    SHAPING_ALIVE_BONUS = 0.15
    SHAPING_TIME_PENALTY = 0.0
    SHAPING_JERK_WEIGHT = 0.002

    SHAPING_PROGRESS_WEIGHT = 6.0
    SHAPING_LANE_WEIGHT = 1.0
    SHAPING_SAFE_WEIGHT = 0.5

    SHAPING_TARGET_LANE_Y = -1.6
    SHAPING_LANE_SCALE = 3.2
    SHAPING_SAFE_DISTANCE = 10.0

    SHAPING_REWARD_NORM_EPS = 1e-6
    SHAPING_REWARD_NORM_DECAY = 0.999
    SHAPING_OUTPUT_SCALE = 0.5
    SHAPING_OUTPUT_OFFSET = 0.02
    SHAPING_OUTPUT_TANH_SCALE = 2.0
    ...
    def load_from_file(cls, filename):
        ...
        for item in contents:
            ...
            elif item in {"MODEL_NAME", "INIT_MODEL_NAME"} and value:
                value = str(resolve_runtime_path(value, search_dirs=[RUNS_DIR, LOGS_DIR]))
