...
def get_before_after_constraints(s_sequence, t_values,s_ego,ego_position):
    #last_ego_position = s_sequence[-1]
    last_ego_position = s_ego
    ...
    car_front, car_behind  = control.get_closest_vehicles(positions,speeds)
    if car_behind is None and car_front is not None:
        after_car_start_pos = -np.inf
        after_car_speed = 0
        before_car_start_pos = control.get_obstacle_s(positions[car_front])
        before_car_speed = speeds[car_front]

    elif car_front is None and car_behind is not None :
        before_car_start_pos = np.inf
        before_car_speed = 0
        after_car_start_pos = -np.inf
        after_car_speed = 0
    elif car_front is None and car_behind is  None:
        after_car_start_pos = -np.inf
        after_car_speed = 0
        before_car_start_pos = np.inf
        before_car_speed = 0
    else:
        after_car_start_pos = control.get_obstacle_s(positions[car_behind])
        after_car_speed = speeds[car_behind]
        before_car_start_pos = control.get_obstacle_s(positions[car_front])
        before_car_speed = speeds[car_front]
