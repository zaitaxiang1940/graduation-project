...
try:
    ...
    class SummaryWriter:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("torch 未安装，无法使用 SummaryWriter。")
...
class RLAgent(ABC):
    ...
    def do_combined_control(self, state: prediction.HighwayState) -> float:
        ...
        ego_position = start_state.ego_position
        s_ego = control.get_ego_s(start_state.ego_position)
        ...
        while not (i >= max(Settings.ROLLOUT_LENGTH, 1)):
            i += 1
            if i != 1:
            ...
            state, crash_predicted = state.predict_step_with_ego(selected_speed, Settings.T_DISCRETIZATION,
        ...
        if Settings.CHECK_ROLLOUT_CRASH and crash_predicted:
            ...
            s_sequence_rl = np.array(rollout_s_history)

            end_point = len(s_sequence_rl)
            while s_sequence_rl[end_point - 1] == 0:
                end_point -= 1
            s_sequence_rl = s_sequence_rl[:end_point]
            step_size = 0.3
            t_rl_values = np.arange(len(s_sequence_rl)) * step_size
            get_before_after_constraints = st.get_before_after_constraints(s_sequence_rl,t_rl_values,s_ego,ego_position)
            ...
            if Settings.TICK_LENGTH < Settings.T_DISCRETIZATION:
                ...
                s_sequence_rl = st.finer_fit(s_sequence_rl, Settings.TICK_LENGTH,Settings.T_DISCRETIZATION,
                                          start_state.ego_speed,
                                          start_state.ego_acceleration,get_before_after_constraints)
            ...
            # return end_speed_first_step
            # self.takeover_history.append(True)
            planned_distance_first_step_rl = s_sequence_rl[1] - s_sequence_rl[0]
            end_speed_first_step_rl = planned_distance_first_step_rl / (Settings.TICK_LENGTH)
            control.set_ego_speed(end_speed_first_step_rl)
            return end_speed_first_step_rl
        else:
            self.takeover_history.append(False)
            return control.set_ego_jerk(first_action)
...
def get_state_vector_from_base_state(state: prediction.HighwayState, cars_ahead=None, cars_behind=None):
    ...
    use_acc = Settings.USE_ACCELERATION_OF_OTHER_CARS
    use_diff = Settings.USE_SPEED_DIFFERENCE
    normalize = Settings.NORMALIZE_VECTOR_INPUT

    dim_per_vehicle = 4 if use_acc else 3
    n_vehicles = cars_ahead + cars_behind
    out = np.zeros(dim_per_vehicle * n_vehicles + 4)

    front = []
    back = []
    other_speeds = state.other_speeds
    other_xs = state.other_xs
    other_accs = state.other_accelerations

    for i in range(len(other_speeds)):
        vehicle_speed = other_speeds[i]
        vehicle_x = other_xs[i]
        dx = vehicle_x - ego_x

        if use_acc:
            a = other_accs[i]
            v = (vehicle_speed - ego_speed) if use_diff else vehicle_speed
            tup = (a, v, dx, 1.0)
        else:
            v = (vehicle_speed - ego_speed) if use_diff else vehicle_speed
            tup = (v, dx, 1.0)
    ...
    if normalize:
        inv9 = 1.0 / 9.0
        inv_max_speed = 1.0 / Settings.MAX_SPEED
        inv_radius = 1.0 / Settings.SENSOR_RADIUS

    write_i = 0
    front_take = min(cars_ahead, len(front))
    for j in range(front_take):
        if use_acc:
            a, v, dx, present = front[j]
            if normalize:
                out[write_i + 0] = a * inv9
                out[write_i + 1] = v * inv_max_speed
                out[write_i + 2] = dx * inv_radius
                out[write_i + 3] = present
            else:
                out[write_i + 0] = a
                out[write_i + 1] = v
                out[write_i + 2] = dx
            ...
            v, dx, present = front[j]
            if normalize:
                out[write_i + 0] = v * inv_max_speed
                out[write_i + 1] = dx * inv_radius
                out[write_i + 2] = present
            else:
                out[write_i + 0] = v
                out[write_i + 1] = dx
                out[write_i + 2] = present
        write_i += dim_per_vehicle

    write_i = dim_per_vehicle * cars_ahead
    back_take = min(cars_behind, len(back))
    for j in range(back_take):
        if use_acc:
            a, v, dx, present = back[j]
            if normalize:
                out[write_i + 0] = a * inv9
                out[write_i + 1] = v * inv_max_speed
                out[write_i + 2] = dx * inv_radius
    ...
    tail = dim_per_vehicle * n_vehicles
    if normalize:
        out[tail + 0] = ego_speed * (1.0 / Settings.MAX_SPEED)
        out[tail + 1] = state.ego_acceleration * (1.0 / 9.0)
        out[tail + 2] = state.ego_position[0] / 300.0
        out[tail + 3] = state.ego_position[1] / 100.0
    else:
        out[tail + 0] = ego_speed
        out[tail + 1] = state.ego_acceleration
        out[tail + 2] = state.ego_position[0]
        out[tail + 3] = state.ego_position[1]
...
def get_reward_function():
    ...
    elif Settings.REWARD_FUNCTION == "Shaped":
        reward_function = ShapedReward()
    elif Settings.REWARD_FUNCTION == "Shaped Stable":
        reward_function = ShapedStableReward()
    else:
...
def continuous_reward(state: prediction.HighwayState, jerk, crashed, arrived):
    ...
    if crashed:
        absolute_metric = Settings.CRASH_REWARD
    elif arrived:
        absolute_metric = Settings.SUCCESS_REWARD
    else:
...
class ShapedReward:
    ...
    def reset(self, state):
        ...
        try:
            self._s0 = float(control.get_ego_s(state.ego_position))
        except Exception:
            self._s0 = 0.0

    def _goal_s(self):
        try:
            return float(control.get_ego_s((Settings.STOP_X, Settings.SHAPING_TARGET_LANE_Y)))
        except Exception:
            return 1.0
    ...
    def _phi(self, state):
        if state is None:
            return 0.0
        try:
            ego_x, ego_y = state.ego_position
        except Exception:
            return 0.0

        s = float(control.get_ego_s((ego_x, ego_y)))
        if self._s0 is None:
            self._s0 = s
        denom = max(1e-6, self._goal_s() - self._s0)
        progress = (s - self._s0) / denom
        progress = float(np.clip(progress, 0.0, 1.0))

        lane_error = abs(float(ego_y) - float(Settings.SHAPING_TARGET_LANE_Y))
        lane = 1.0 - min(lane_error / max(1e-6, float(Settings.SHAPING_LANE_SCALE)), 1.0)

        safe = 0.0
        if s > 0:
            car_ahead, car_behind = state.get_closest_cars()
            if car_ahead is not None:
                ahead_x, _, _ = car_ahead
                front_distance = ahead_x - ego_x - Settings.CAR_LENGTH
            else:
                front_distance = np.inf
            if car_behind is not None:
                behind_x, _, _ = car_behind
                back_distance = ego_x - behind_x - Settings.CAR_LENGTH
            else:
                back_distance = np.inf
            min_distance = min(front_distance, back_distance)
            if min_distance == np.inf or np.isnan(min_distance):
                safe = 1.0
            else:
                safe = float(np.clip(min_distance / max(1e-6, float(Settings.SHAPING_SAFE_DISTANCE)), 0.0, 1.0))

        return (
            float(Settings.SHAPING_PROGRESS_WEIGHT) * progress
            + float(Settings.SHAPING_LANE_WEIGHT) * float(lane)
            + float(Settings.SHAPING_SAFE_WEIGHT) * safe
        )

    def __call__(self, state, jerk, crashed, arrived):
        if crashed:
            self._prev_state = None
            self._s0 = None
            return float(Settings.CRASH_REWARD)
        if arrived:
            self._prev_state = None
            self._s0 = None
            return float(Settings.SUCCESS_REWARD)

        dt = float(Settings.TICK_LENGTH)
        base = (float(Settings.SHAPING_ALIVE_BONUS) - float(Settings.SHAPING_TIME_PENALTY)) * dt
        comfort = -float(Settings.SHAPING_JERK_WEIGHT) * float(jerk) ** 2 * dt

        phi_prev = self._phi(self._prev_state)
        phi_next = self._phi(state)
        shaping = float(Settings.SHAPING_GAMMA) * phi_next - phi_prev

        self._prev_state = state
        return base + comfort + shaping


class ShapedStableReward(ShapedReward):
    def __init__(self):
        super().__init__()
        self._mean = 0.0
        self._var = 1.0
    ...
    def _update_running_stats(self, x: float):
        decay = float(Settings.SHAPING_REWARD_NORM_DECAY)
        x = float(x)
        mean = self._mean
        var = self._var
        delta = x - mean
        ...
        self._mean = mean
        self._var = max(float(Settings.SHAPING_REWARD_NORM_EPS), float(var))

    def __call__(self, state, jerk, crashed, arrived):
        raw = float(super().__call__(state, jerk, crashed, arrived))
        self._update_running_stats(raw)
        std = float(np.sqrt(self._var))
        z = (raw - self._mean) / (std + float(Settings.SHAPING_REWARD_NORM_EPS))
        delta = std * float(Settings.SHAPING_OUTPUT_SCALE) * float(
            np.tanh(z / max(float(Settings.SHAPING_OUTPUT_TANH_SCALE), 1e-6))
        )
        y = float(self._mean) + float(delta) + float(Settings.SHAPING_OUTPUT_OFFSET)
        return y
...
class SumTree:
    ...
    def __init__(self, capacity):
        ...
        self.samples = np.zeros(self.capacity, dtype=object)
        self.size = 0
...
def train_dqn_all():
    ...
    copytree(default_log_dir, Settings.FULL_LOG_DIR, dirs_exist_ok=True)
