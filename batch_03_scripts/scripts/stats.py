...
class StatsAggregator:
    ...
    def get_stat_report_row_dict(self):
        ...
        if pd is not None:
            columns["TIME"] = str(pd.Timestamp.now(tz="US/Pacific"))
        else:
            import datetime

            columns["TIME"] = datetime.datetime.now().isoformat()
        return columns

    def add_csv_data(self):
        row = self.get_stat_report_row_dict()
        if pd is None:
            file_exists = os.path.exists("run_data.csv")
            with open("run_data.csv", "a", newline="") as f:
                writer = DictWriter(f, fieldnames=list(row.keys()))
                if not file_exists:
                    writer.writeheader()
                writer.writerow(row)
            return
