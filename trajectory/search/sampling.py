...
def _sample_from_logits(logits, temperature=1.0, topk=None, cdf=None):
...
def sample(model, x, temperature=1.0, topk=None, cdf=None, **forward_kwargs):
    ...
    logits = forward(model, x, **forward_kwargs)
    logits = logits[:, -1]
    return _sample_from_logits(logits, temperature=temperature, topk=topk, cdf=cdf)
...
def sample_n(model, x, N, **sample_kwargs):
    ...
    max_block = sample_kwargs.get("max_block", None)
    crop_increment = sample_kwargs.get("crop_increment", None)
    ...
    for n in range(N):
        indices, p = _sample_from_logits(
            logits_last,
            temperature=sample_kwargs.get("temperature", 1.0),
            topk=sample_kwargs.get("topk", None),
            cdf=sample_kwargs.get("cdf", None),
        )
        ...
        if n < N - 1:
            out = model(indices, use_cache=True, past_key_values=past, start_pos=past_len)
            logits_last = out[0][:, -1]
            past = out[2]
            past_len += 1
