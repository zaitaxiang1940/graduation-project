...
def beam_search(model, x, n_steps, beam_width=512, goal=None, **sample_kwargs):
    ...
    transition_dim =23 #observation_dim + action_dim + REWARD_DIM + VALUE_DIM
    max_block = 114#max_context_transitions * transition_dim - 1 if max_context_transitions else None
    sample_kwargs = {
        'max_block': max_block,
        'crop_increment': transition_dim,
    }
