...
class QuantileDiscretizer:
	...
	def _test(self):
		...
		stable_dims = (self.diffs.max(axis=0) > 0)
		if stable_dims.any():
			pass  # assert (indices[:, stable_dims] == indices_2[:, stable_dims]).all()
	...
	def reconstruct(self, indices, subslice=(None, None)):
		...
		max_bins = thresholds.shape[0] - 1
		max_index = min(self.N - 1, max_bins - 1)
		if indices.min() < 0 or indices.max() > max_index:
	...
	def expectation(self, probs, subslice):
		...
		if probs.shape[-1] != n_bins:
			if probs.shape[-1] > n_bins:
				probs = probs[..., :n_bins]
			else:
				pad_width = n_bins - probs.shape[-1]
	...
	def percentile(self, probs, percentile, subslice):
		...
		## [ B ]
		inds = np.argmax(np.arange(n_bins, 0, -1) * valid, axis=-1)
