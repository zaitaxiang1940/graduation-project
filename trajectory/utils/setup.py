...
try:
    ...
    try:
        from tap import TapCommand as Tap
...
class Parser(Tap if Tap is not None else object):
    ...
    def parse_args(self, experiment=None):
        if Tap is None:
            raise RuntimeError(
                "参数解析依赖缺失/冲突：无法导入 typed-argument-parser 的 tap 模块。"
                "请先执行：pip uninstall -y tap && pip install typed-argument-parser"
            ) from _TAP_IMPORT_ERROR
